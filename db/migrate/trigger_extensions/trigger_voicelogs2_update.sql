--TriggerName=add_voicelogs_2_statistics_afup
 
CREATE TRIGGER add_voicelogs_2_statistics_afup AFTER UPDATE ON voice_logs_today_2
FOR EACH ROW BEGIN
	...
	IF get_active_logger_id(2) THEN
		IF (new.ori_call_id = '1' or new.ori_call_id = '' or new.ori_call_id is NULL or new.ori_call_id = 1) THEN
			-- main call
		...
		ELSE
			...
			FROM voice_logs_today_2 WHERE call_id = new.ori_call_id LIMIT 1;
		END IF;
		...
		IF xmain_callid IS NOT NULL AND xori_id IS NOT NULL THEN
			...
			FROM voice_logs_today_2 WHERE ori_call_id = xmain_callid;	
