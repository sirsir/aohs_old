class CreateTriggerModifyStatisticWhenResultChange < ActiveRecord::Migration
  def self.up
#   execute "DROP TRIGGER IF EXISTS modify_statistics_when_result_change"
   execute <<SQL
   CREATE TRIGGER modify_statistics_when_result_change AFTER UPDATE ON result_keywords
    FOR EACH ROW BEGIN
