...
FOR EACH ROW BEGIN
	...
	SELECT id INTO counter_id FROM voice_log_counters WHERE voice_log_id = new.voice_log_id;
	IF counter_id IS NOT NULL THEN
		UPDATE voice_log_counters SET bookmark_count = bookmark_count + 1 WHERE voice_log_id = new.voice_log_id LIMIT 1;
	END IF;
END;