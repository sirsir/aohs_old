...
FOR EACH ROW BEGIN
	...
	IF new.keyword_id IS NOT NULL THEN
		...
		IF k_type IS NOT NULL AND r_time IS NOT NULL THEN
			...
			ELSE
				SET daily_value = 1;
				INSERT INTO daily_statistics (start_day,keyword_id,statistics_type_id,VALUE,created_at,updated_at) VALUES (r_time,new.keyword_id,s2_type,daily_value,NOW(),NOW());
			END IF;
		END IF;
	END IF;
	--   END IF;
