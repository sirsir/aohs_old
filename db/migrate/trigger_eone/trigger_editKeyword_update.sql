...
FOR EACH ROW BEGIN
	...
	ELSE
		SELECT id,CAST(start_time AS DATE),agent_id INTO xvoice_id,xdate,xuser_id FROM voice_logs_2 WHERE id = new.voice_log_id LIMIT 1;
	END IF;
	...
	IF xkeyword_type IS NOT NULL AND xdate IS NOT NULL AND xvoice_id IS NOT NULL THEN
		...
		IF new.edit_status = 'd' AND (old.edit_status != new.edit_status) THEN
			...
			IF xstatistic_type_id2 IS NOT NULL THEN
				UPDATE daily_statistics d SET value = value - 1 
