--TriggerName=add_voicelogs_statistics_afup
 
CREATE TRIGGER add_voicelogs_statistics_afup AFTER UPDATE ON voice_logs_today
FOR EACH ROW BEGIN
	...
	ELSE
		...
		FROM voice_logs_today WHERE call_id = new.ori_call_id LIMIT 1;
	END IF;
	...
	IF xmain_callid IS NOT NULL AND xori_id IS NOT NULL THEN
		...
		FROM voice_logs_today WHERE ori_call_id = xmain_callid;	
