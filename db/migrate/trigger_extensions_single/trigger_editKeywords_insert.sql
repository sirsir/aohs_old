--TriggerName=count_statistics_keyword_for_edit_insert
 
CREATE TRIGGER count_statistics_keyword_for_edit_insert AFTER INSERT ON edit_keywords
FOR EACH ROW BEGIN
	...
	IF new.keyword_id IS NOT NULL THEN
		...
		IF new.edit_status = 'n' OR new.edit_status IS NULL THEN
			...
			FROM voice_logs WHERE id = new.voice_log_id LIMIT 1;
			...
			IF xkeyword_type IS NOT NULL AND xdate IS NOT NULL AND xvoice_id IS NOT NULL THEN
				...
				IF xstatistic_type_id1 IS NOT NULL THEN
					...
					ELSE
						UPDATE daily_statistics SET value = value + 1, updated_at = NOW() 
						WHERE id = xstatistic_id1;
					END IF;
				END IF;
				...
				IF xstatistic_type_id2 IS NOT NULL THEN
					...
					ELSE
						UPDATE daily_statistics SET value = value + 1, updated_at = NOW() 
						WHERE id = xstatistic_id2 LIMIT 1;
					END IF;
				END IF;
