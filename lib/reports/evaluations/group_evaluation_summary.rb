module EvaluationReport
  class GroupEvaluationSummary < EvaluationReportBase

    def initialize(opts={})
      @opts = opts
      @opts[:report_name] = "Group Summary Report"
      initial_report
      initial_header
      ...
      result.each do |rs|
        ...
        @col_quests.each do |matched_id|
          ob = (result_scores.select { |x|
            x["matched_id"].to_i == matched_id and rs["group_id"].to_i == x["group_id"].to_i
          }).first
          if ob.blank?
            d << nil
          else
        ...
        if @opts[:calc] == "total"
          ...
          d[2] = get_grade_v2(cscore, rs["grade_list"])
          d[3] = StringFormat.score_fmt(cscore)
        else
    ...
    def sql_detail_by_question
      sql = []
      sql << "SELECT l.id, l.group_id"
      sql << "FROM evaluation_logs l"
      sql << "JOIN evaluation_plans p ON l.evaluation_plan_id = p.id"
      sql << "JOIN evaluation_calls c ON c.evaluation_log_id = l.id"
      sql << "WHERE #{jn_where(get_where)}"
      sql << "GROUP BY l.id, l.group_id"
      sql1 = jn_sql(sql)
      sql = []
      sql << "SELECT l.group_id AS group_id,"
      ...
      sql << "GROUP BY l.group_id, d.question_id"
      return jn_sql(sql)
    end
    ...
    def sql_detail_by_group_question
      ...
      sql << "JOIN evaluation_score_logs s ON l.id = s.evaluation_log_id"
      sql << "LEFT JOIN evaluation_question_display d ON s.evaluation_question_id = d.question_id"
      sql << "GROUP BY l.group_id, d.question_group_id"
      return jn_sql(sql)
    end
